(() => {
  ...
  function formatByCurrency(value) {
    ...
    const loc = lang === 'ru' ? 'ru-RU' : lang === 'ka' ? 'ka-GE' : lang === 'da' ? 'da-DK' : 'en-US';
    const n = Number(value || 0);
    const val = n.toLocaleString(loc, { minimumFractionDigits: n % 1 === 0 ? 0 : 2, maximumFractionDigits: 2 });
  ...
  style.textContent = `
    ...
    body.theme-dark .listora-currency-select,body:not(.theme-light) .listora-currency-select{background:#0f172a;color:#f8fafc;border-color:#334155}
    .listora-language-currency-row .language-current{min-height:48px;padding:11px 12px}
  ...
  function languageWord() {
    ...
    if (lang === 'da') return 'Sprog';
    if (lang === 'ka') return 'ენა';
    return 'Language';
  }
  ...
  function enhanceSettingsCurrency() {
    ...
    if (!languagePicker || sheet.dataset.currencyReady === '1') return;
    const block = languagePicker.closest('.settings-block');
    if (!block) return;
    sheet.dataset.currencyReady = '1';

    const title = block.querySelector('.settings-title-small');
    if (title) title.innerHTML = `🌍 ${languageWord()} и ${currencyWord()}`;
